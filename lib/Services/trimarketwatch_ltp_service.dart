...
class TriMarketWatchLtpService with ChangeNotifier {
  ...
  setIts(String it, String cid) {
    DateTime currentTime = DateTime.now();
    double currentTimeDouble =
        currentTime.hour.toDouble() + (currentTime.minute.toDouble() / 60);

    if (currentTimeDouble > openingTime &&
        currentTimeDouble < closingTime &&
  ...
  fetchBrokerLtp(String it, String exchange, String tradingsymbol) async {
    Uri url = Uri.parse('http://15.207.48.223:3300/getserverltp');
